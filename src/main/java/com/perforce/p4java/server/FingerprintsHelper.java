...
public class FingerprintsHelper extends AbstractAuthHelper {
	 ...
	 * @param trustFilePath trustFilePath
	 * @return - fingerprint value or null if not found
	 * @throws IOException - io exception from reading trust file
	 */
	...
	public static Fingerprint[] getFingerprints(File trustFile) throws IOException {
		...
		List<Map<String, String>> authList = trustFile != null ? getFileEntries(trustFile) : getMemoryEntries(fingerprintsMap);
		if (authList != null) {
	 ...
	 *
	 * @param userName      - possibly null user name to match against the found
	 *                      fingerprints
	 * @param serverAddress - non-null server address
	 * @param trustFilePath - path to trust file to search
	...
	public static Fingerprint getFingerprint(String userName, String serverAddress, String trustFilePath) throws IOException {
		...
		if (serverAddress != null) {
			...
			for (Fingerprint fingerprint : getFingerprints(trustFilePath)) {
				if (serverAddress.equals(fingerprint.getServerAddress()) && (userName == null || userName.equals(fingerprint.getUserName()))) {
	 ...
	 *
	 * @param fingerprint   - non-null fingerprint
	 * @param trustFilePath - non-null path
	 ...
	 *
	 * @param fingerprint - non-null fingerprint
	 * @param trustFile   - non-null file
	 * @throws IOException on error
	 */
	public static void saveFingerprint(Fingerprint fingerprint, File trustFile) throws IOException {
		if (fingerprint != null) {
			saveFingerprint(fingerprint.getUserName(), fingerprint.getServerAddress(), fingerprint.getFingerprintValue(), trustFile);
		}
	}
	 ...
	 * @param serverAddress    - non-null server address
	 * @param fingerprintValue - non-null fingerprint value
	 * @param trustFilePath    - non-null file path
	 * @throws IOException on error
	 */
	...
	public static void saveFingerprint(String userName, String serverAddress, String fingerprintValue, String trustFilePath, int lockTry, long lockDelay, long lockWait) throws IOException {
		File file = trustFilePath != null ? new File(trustFilePath) : null;
		saveFingerprint(userName, serverAddress, fingerprintValue, file, lockTry, lockDelay, lockWait);
	}
	 ...
	 * @param userName         - non-null user name
	 * @param serverAddress    - non-null server address
	 * @param fingerprintValue - possibly null fingerprint value
	 * @param trustFile        - non-null file
	 * @throws IOException on error
	 */
	...
	public static void saveFingerprint(String userName, String serverAddress, String fingerprintValue, File trustFile, int lockTry, long lockDelay, long lockWait) throws IOException {
		if (trustFile != null) {
			saveFileEntry(userName, serverAddress, fingerprintValue, trustFile, lockTry, lockDelay, lockWait);
		} else {
