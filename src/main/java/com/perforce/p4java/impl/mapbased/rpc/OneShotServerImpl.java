...
public class OneShotServerImpl extends RpcServer {
			...
			ShutdownHandler handler = new ShutdownHandler() {
				...
				public void shutdown(Socket socket) {
					try {
						...
						RpcStreamConnection rpcConnection = new RpcStreamConnection(serverHost, serverPort, OneShotServerImpl.this.props, OneShotServerImpl.this.serverStats, OneShotServerImpl.this.p4Charset, socket, null, OneShotServerImpl.this.secure, OneShotServerImpl.this.rsh);
	...
	protected List<Map<String, Object>> execMapCmdList(String cmdName, String[] cmdArgs, Map<String, Object> inMap, String inString, boolean ignoreCallbacks, IStreamingCallback callback, int callbackKey, IFilterCallback filterCallback, IParallelCallback parallelCallback) throws ConnectionException, AccessException, RequestException {
		...
		try {
			...
			ExternalEnv env = setupCmd(dispatcher, rpcConnection, protocolSpecs, cmdName.toLowerCase(Locale.ENGLISH), cmdArgs, inMap, ignoreCallbacks, cmdCallBackKey, false);
			...
			if (this.caseSensitive && cmdEnv.getServerProtocolSpecsMap().containsKey(RpcFunctionMapKey.NOCASE)) {
	 ...
	 * @param protocolSpecs   protocolSpecs
	 * @param cmdName         cmdName
	 * @param cmdArgs         cmdArgs
	 * @param inMap           inMap
	 * @param ignoreCallbacks ignoreCallbacks
	 * @param cmdCallBackKey  cmdCallBackKey
	...
	protected ExternalEnv setupCmd(RpcPacketDispatcher dispatcher, RpcConnection rpcConnection, ProtocolCommand protocolSpecs, String cmdName, String[] cmdArgs, Map<String, Object> inMap, boolean ignoreCallbacks, int cmdCallBackKey, boolean isStream) throws ConnectionException, AccessException, RequestException {
		...
		if (props.containsKey(RpcFunctionMapKey.IPADDR) && props.containsKey(RpcFunctionMapKey.SVRNAME) && props.containsKey(RpcFunctionMapKey.PORT)) {
