...
public class RpcPacketDispatcher {
	...
	public enum RpcPacketDispatcherResult {
		NONE, CONTINUE,        // continue without changing dispatch mode
	 ...
	 * the implementation here may be less than transparent...
	 * <p>
	 ...
	 */
	public List<Map<String, Object>> dispatch(CommandEnv cmdEnv) throws ConnectionException, AccessException {
		...
		try {
			...
			while ((packet = rpcConnection.getRpcPacket(cmdEnv.getFieldRule(), cmdEnv.getFilterCallback())) != null) {
				...
				switch (func.getType()) {
					...
					case CLIENT:
						switch (clientDispatcher.dispatch(dispatchMode, func, cmdEnv, paramMap)) {
							case CONTINUE:
								break;
							case CONTINUE_LOOP:
					...
					case PROTOCOL:
						switch (protocolDispatcher.dispatch(dispatchMode, func, cmdEnv, paramMap)) {
							case CONTINUE:
								break;
							case CONTINUE_LOOP:
								// We're in (or starting) an interactive exchange...
