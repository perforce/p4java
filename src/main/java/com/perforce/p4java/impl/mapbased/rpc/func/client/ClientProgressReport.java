...
public class ClientProgressReport {
		...
		if (resultsMap != null) {
			...
			switch (funcSpec) {
				...
				case CLIENT_OUTPUTTEXT:
					return progressCallback.tick(cmdCallBackKey, tickMarker);
				case CLIENT_MESSAGE:
					tickMarker = ResultMapParser.getErrorOrInfoStr(resultsMap);
					return progressCallback.tick(cmdCallBackKey, tickMarker);
				default:
					break;
			}
	...
	private String getTickMarker(CmdSpec cmdSpec, Map<String, Object> resultsMap) {
		...
		if (cmdSpec != null) {
			...
			switch (cmdSpec) {
				case FSTAT:
					final String[] fstatFields = {RpcFunctionMapKey.DEPOT_FILE, RpcFunctionMapKey.CLIENT_FILE, RpcFunctionMapKey.ISMAPPED, RpcFunctionMapKey.HEADACTION, RpcFunctionMapKey.HEADTYPE, RpcFunctionMapKey.HEADTIME, RpcFunctionMapKey.HEADREV, RpcFunctionMapKey.HEADCHANGE, RpcFunctionMapKey.HEADMODTIME, RpcFunctionMapKey.HAVEREV};
