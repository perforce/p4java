...
public class RpcPacketField {
	...
	public static Object[] retrievePacketField(ByteBuffer buf, boolean isUnicodeServer, Charset charset, RpcPacketFieldRule fieldRule) {
		...
		if (i > 0) {
			...
			try {
				retVal[NAME_FIELD] = new String(bytes, 0, i, charset == null ? RpcConnection.NON_UNICODE_SERVER_CHARSET_NAME : (isUnicodeServer ? CharsetDefs.UTF8_NAME : charset.name()));
		...
		if (!skipConversion && RpcPacketFieldType.getFieldType((String) retVal[NAME_FIELD]) == RpcPacketFieldType.TEXT) {
			...
			try {
				retVal[VALUE_FIELD] = new String(valBytes, charset == null ? RpcConnection.NON_UNICODE_SERVER_CHARSET_NAME : (isUnicodeServer ? CharsetDefs.UTF8_NAME : charset.name()));
			} catch (UnsupportedEncodingException e) {
				// This should never be reached since we already have the Charset
				// object needed.
				Log.exception(e);
	 ...
	 * @throws BufferOverflowException      on error
	 */
	public static void marshal(ByteBuffer buf, String name, String value, Charset charset) throws BufferOverflowException, UnsupportedEncodingException {
		marshal(buf, name, (value == null ? null : value.getBytes(charset == null ? CharsetDefs.DEFAULT.name() : charset.name())));
	}
	...
	public static void marshal(ByteBuffer buf, String name, StringBuffer value, Charset charset) throws BufferOverflowException, UnsupportedEncodingException {
		marshal(buf, name, (value == null ? null : value.toString().getBytes(charset == null ? CharsetDefs.DEFAULT.name() : charset.name())));
	}
