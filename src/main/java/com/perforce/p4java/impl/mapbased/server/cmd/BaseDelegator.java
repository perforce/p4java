...
public abstract class BaseDelegator {
	 ...
	 *
	 * @param map    The result map from an execcmd operation
	 * @param client client
	 * @return An IFileSpec object
	 * @throws AccessException     Insufficient privileges for the server
	 * @throws ConnectionException Cannot connect to the server
	 * @deprecated use @deprecated use {@link ResultListBuilder#handleFileReturn(Map, IServer)} instead
	 */
	@Deprecated
	IFileSpec handleFileReturn(final Map<String, Object> map, final IClient client) throws AccessException, ConnectionException {
		if (map != null) {
	 ...
	 * @param inMap   Optional, used to provide input data to the server command;
	 *                e.g. spec data
	 ...
	 */
	List<Map<String, Object>> execMapCmdList(@Nonnull final CmdSpec cmdSpec, String[] cmdArgs, Map<String, Object> inMap) throws ConnectionException, AccessException, RequestException {
	...
	void checkMinSupportedPerforceVersion(final String userName, final int maxLimit, final String queryString, final String clientOrLabel) throws RequestException, ConnectionException {
		...
		if (isNotBlank(userName)) {
			throwRequestExceptionIfPerforceServerVersionOldThanExpected(serverVersion >= USER_RESTRICTIONS_SUPPORTED_MIN_VERSION, "user restrictions '%s' for %s lists are not supported by this version of the Perforce server", userName, clientOrLabel);
		}
		if (maxLimit > 0) {
			throwRequestExceptionIfPerforceServerVersionOldThanExpected(serverVersion >= MAX_LIMIT_SUPPORTED_MIN_VERSION, "max limit '%s' for %s lists are not supported by this version of the Perforce server", maxLimit, clientOrLabel);
		}
		if (isNotBlank(queryString)) {
			throwRequestExceptionIfPerforceServerVersionOldThanExpected(serverVersion >= QUERY_EXPRESSIONS_SUPPORTED_MIN_VERSION, "query expressions '%s' for %s lists are not supported by this version of the Perforce server", queryString, clientOrLabel);
		}
	}
}